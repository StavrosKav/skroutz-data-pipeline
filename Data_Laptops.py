...
Data_Laptops.py
---------------
Cleans and enriches the raw laptop data produced by skroutz_laptopsWHILE.py.

Reads:  Laptops_skroutz/skroutz_laptos_<today>.csv
Writes: Clean/Laptops_skroutz_clean/clean_<today>.csv

Transformations applied:
  - Price:        handles Greek thousands-separator format (e.g. "1.727,75" → 1727.75)
  - Brand/Model:  split from the product name
  - Installments: converts Greek decimal comma format ("77,83") to float
"""
...
def clean_price(val):
    ...
    and ',' is the decimal separator (e.g. "1.727,75 €" = 1727.75 euros).
...
if __name__ == "__main__":
    ...
    base_folder = os.path.join(BASE, 'Laptops_skroutz')
    file_path = os.path.join(base_folder, f"skroutz_laptos_{today}.csv")
    ...
    # ── BRAND / MODEL ─────────────────────────────────────────────────────────
    # Laptop product names follow the structure: "<Brand> <Model>"
    # e.g. "Apple MacBook Air 15.3" M4" or "Lenovo IdeaPad 1 15IAU7"
    #
    # pattern_full tries to capture color/variant after a (RAM/StorageGB) spec block
    # (some premium laptops include this in the title).
...
"""
    ...
    # Raw installment values use Greek decimal commas (e.g. "77,83" = 77.83 €).
    ...
    output_folder = os.path.join(BASE, 'Clean', 'Laptops_skroutz_clean')
