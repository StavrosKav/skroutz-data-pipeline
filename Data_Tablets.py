...
Transformations applied:
  ...
  - Installments: converts Greek decimal comma format ("25,50") to float
                  (column added here; populated once the updated scraper is used)
"""
...
# Installments columns were added to the scraper in the updated version.
# For older CSVs that lack these columns, they are initialised as NaN here so the
# output schema stays consistent and 4csvsTOsql.py can always find them.
for col in ['Installments_per_month', 'Installments_in_total']:
    if col not in data.columns:
        data[col] = None   # older scraper didn't capture installments
    else:
