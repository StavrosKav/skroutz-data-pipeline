...
def _header():
    ...
    c3.metric("Price drops today", f"{total_drops or 0}")
    c4.metric("Last updated", str(last_updated) if last_updated else "—")
...
def tab_overview():
    ...
    for _, row in cat_df.iterrows():
        with st.expander(CAT_LABEL.get(row.category, row.category), expanded=True):
            a, b, c, d = st.columns(4)
            a.metric("Products", f"{int(row.products):,}")
            b.metric("Avg price", _fmt_eur(row.avg_price))
            c.metric("Min price", _fmt_eur(row.min_price))
            d.metric("Max price", _fmt_eur(row.max_price))

    st.subheader("Brand price trends")
    cat_filter = st.selectbox("Category", CATEGORIES, format_func=lambda c: CAT_LABEL[c], key="ov_cat")
    ...
    if chart_path.exists():
        ...
        if not trend_df.empty:
            pivot = trend_df.pivot(index="date", columns="brand", values="avg_price")
            st.line_chart(pivot)
        else:
...
def tab_drops():
    ...
    drops_df = _q(f"""
        SELECT brand, model, category,
               ...
               ROUND(drop_eur,   2) AS "Saved €",
               ROUND(drop_pct,   1) AS "Drop %",
               drop_date,
               skroutz_link
    ...
    if drops_df.empty:
        ...
        st.dataframe(
            drops_df.drop(columns=["skroutz_link"]),
    ...
    if hot_df.empty:
        ...
        st.dataframe(
            hot_df.drop(columns=["skroutz_link"]),
            use_container_width=True,
            hide_index=True,
        )
...
def tab_products():
    ...
    if cat_f:
        ...
        for i, c in enumerate(cat_f):
            params[f"c{i}"] = c
    if brand_search:
        cat_sql += " AND LOWER(lp.brand) LIKE :brand"
    ...
    st.write(f"**{len(prod_df):,}** products matching filters")
    if not prod_df.empty:
        st.dataframe(
            prod_df.drop(columns=["skroutz_link"]),
            use_container_width=True,
            hide_index=True,
        )
...
def tab_watchlist():
    ...
    for item in items:
        ...
        with st.expander(f"{'🎯 ' if not row_df.empty and float(row_df.iloc[0]['price_eur'] or 9999) <= threshold else ''}{label}", expanded=True):
            if row_df.empty:
                st.warning("Not found in DB — URL may not match.")
                continue

            price = float(row_df.iloc[0]["price_eur"] or 0)
            delta = price - threshold
            delta_str = f"{delta:+.2f} €"

            c1, c2, c3 = st.columns(3)
            c1.metric("Current price", f"€{price:,.2f}", delta_str,
                      delta_color="inverse")
            c2.metric("Your target", f"€{threshold:,.2f}")
            c3.metric("Status", "✅ Buy now!" if price <= threshold else "⏳ Waiting")

            # Sparkline from vw_price_history
            try:
                hist_df = _q("""
                ...
                if not hist_df.empty and len(hist_df) > 1:
                    ...
                    st.line_chart(hist_df["price"], height=120, use_container_width=True)
            except Exception:
                pass

            st.caption(f"[View on Skroutz]({url})")
...
def tab_analytics():
    ...
    try:
        ...
        if not disc_df.empty:
            st.markdown("**Discount frequency** — how often each brand has a price drop")
    ...
    try:
        vel_df = _q("""
            SELECT brand, model,
                   rev_now AS "Reviews", new_reviews_14d AS "New (14d)",
                   ROUND(reviews_per_day, 2) AS "Rev/day",
                   skroutz_link
        ...
        """, cat=cat_a)
        if not vel_df.empty:
            st.markdown("**Review velocity** — fastest-growing products by review count")
            st.dataframe(
                vel_df.drop(columns=["skroutz_link"]),
                use_container_width=True, hide_index=True,
            )
    except Exception:
        st.info("Review velocity view not yet available.")

    # --- Restock pricing ---
    try:
        restock_df = _q("""
            SELECT brand, model, gap_days,
                   ROUND(price_before, 2) AS "Before €",
                   ROUND(price_after,  2) AS "After €",
                   ROUND(price_chg_pct, 1) AS "Δ%"
        ...
        if not restock_df.empty:
            st.markdown("**Restock pricing** — price change after a product went out of stock")
            st.dataframe(restock_df, use_container_width=True, hide_index=True)
    except Exception:
        st.info("Restock pricing view not yet available.")
    ...
    if dis_df.empty:
        ...
        st.dataframe(
            dis_df.drop(columns=["skroutz_link"]),
            use_container_width=True, hide_index=True,
        )
